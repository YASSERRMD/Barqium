...
impl SnapshotReader {
    ...
    /// Upstreams whose health state is `false` in `health` are skipped.
    ///
    /// # Safety
    ...
    pub fn match_route(
        ...
        let snapshot = &*result;
        Ok(find_route(snapshot, method, path, host, health))
    }
}

/// Finds the first enabled route that matches method, path, and host,
/// skipping upstreams marked unhealthy in `health`.
/// Priority: longest path_prefix wins.
fn find_route(
    ...
    for route in snapshot.routes.iter() {
        ...
        let upstream_id = route.upstream_id.as_str();
        if let Some(h) = health {
            if !h.is_healthy(upstream_id) {
                continue;
            }
        }

        if let Some(upstream) = snapshot.upstream_by_id(upstream_id) {
